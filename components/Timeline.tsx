      ...
      <div className="space-y-16">
        {timeline.map((item, index) => (
            ...
            <span className="text-sm md:text-base tracking-wide uppercase">
              ...
              {/* Certificate button - only for Best Project items */}
              {item.certificate && (
                <a
                    href={item.certificate}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 bg-white text-black px-6 py-3 rounded-full font-jost font-normal text-sm hover:bg-gray-200 transition-colors"
                >
                    VIEW CERTIFICATE
                </a>
            )}
